...
class QuestionBattle_1 extends StatefulWidget {
  QuestionBattle_1({super.key, required this.id,required this.listQuestions});
  int id;
  List<Question> listQuestions;
  @override
  State<QuestionBattle_1> createState() => _QuestionBattleState_1();
}

class _QuestionBattleState_1 extends State<QuestionBattle_1>
  ...
  void _startTimer() {
    timer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (seconds > 0) {
        ...
        fireDb().getPlay(widget.id, false);
        Future.delayed(const Duration(seconds: 1),() => 
        Navigator.push(context, 
        MaterialPageRoute(builder: (context)=>BattleResult_1(id: widget.id,))));
      } else {
        timer!.cancel();
        setState(() {
          isLock = true;
                                                        ...
                                                        ? seconds >= 27
                                                            ...
                                                            : seconds >= 25
                                                                ...
                                                                : seconds >= 20
                                                                    ...
                                                                    : diem += 10
                                                        : null;
                                                      fireDb().updateScore(widget.id, 1, diem);
                                                    _controller.repeat();
                                                    if (_currentIndex == 9) {
                                                      timer!.cancel();
                                                      setState((){
                                                      ...
                                                      fireDb().getPlay(widget.id, false);
                                                      Future.delayed(const Duration(seconds: 1),() => 
                                                        Navigator.push(context, MaterialPageRoute(builder: (context)=>BattleResult_1(id: widget.id,))));
                                                    } else {
                                                      timer!.cancel();
                                                      setState(() {
                                                        isLock = true;
